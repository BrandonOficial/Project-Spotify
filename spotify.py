...
print((dados_musicas.isnull().sum() / dados_musicas.shape[0]).sort_values(ascending=False))

print(dados_musicas.isnull().sum())
...
print(dados_musicas.isna().sum())
print(dados_musicas.isna().sum())

print(dados_musicas.columns)
print(dados_musicas.duration_ms)
...
dados_musicas_original = dados_musicas.copy()

# Remover colunas não numéricas do DataFrame
dados_musicas = dados_musicas.select_dtypes(include=['float64', 'int64'])

# Calcular a matriz de correlação
corr_table = dados_musicas.corr(method="pearson")

# Plotar o mapa de calor da correlação usando seaborn
...
# Acessar as colunas não numéricas da cópia do DataFrame original
colunas_nao_numericas = dados_musicas_original.select_dtypes(exclude=['float64', 'int64',])

# Utilizar as colunas não numéricas quando necessário
# por exemplo:
dados_musicas = pd.concat([dados_musicas, colunas_nao_numericas], axis=1)


#Removendo itens duplicados
dados_musicas = dados_musicas.drop_duplicates()
print(dados_musicas.head())
...
# Converter o objeto para uma representação de string legível
model_str = pickletools.dis(model)

# Salvar a string em um arquivo de texto
with open("RandomForestModel.pkl", "w") as file:
